...
{
    [Info("Magic Air Event Panel", "MJSU edit MeinRust", "1.0.0")]
    [Description("Displays if the Air event is active and changes the color of the Air Event icon when a Air Event is active.")]
    public class MagicHarborEventPanel : RustPlugin
    {
        #region Class Fields
        [PluginReference] private readonly Plugin MagicPanel, AirEvent;

        private PluginConfig _pluginConfig; //Plugin Config
        private int _activeAirEvent;
        ...
        private PluginConfig AdditionalConfig(PluginConfig config)
        {
            config.Panel = new Panel
            {
                Image = new PanelImage
                {
                    ...
                    Url = config.Panel?.Image?.Url ?? "https://www.dropbox.com/s/3bru3m6q9cxae3a/AirEvent.png?dl=1",
        ...
        private void MagicPanelRegisterPanels()
        {
            ...
            if (AirEvent == null)
            {
                PrintError("Missing plugin dependency AirEvent");
                return;
            }
        ...
        {
            _activeAirEvent = Mathf.Min(0, _activeAirEvent - 1);
            if (_activeAirEvent == 0)
            {
                MagicPanel?.Call("UpdatePanel", Name, (int)UpdateEnum.Image);
            }
        }
        ...
        private Hash<string, object> GetPanel()
        {
            ...
            if (image != null)
            {
                image.Color = _activeAirEvent != 0 ? _pluginConfig.ActiveColor : _pluginConfig.InactiveColor;
            }
