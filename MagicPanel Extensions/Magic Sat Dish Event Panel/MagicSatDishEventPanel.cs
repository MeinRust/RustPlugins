...
{
    [Info("Magic Satellite Dish Event Panel", "MJSU edit by MeinRust", "1.0.0")]
    [Description("Displays if the Satellite Dish event is active and changes the color of the Satellite Dish icon when a Satellite Dish is active.")]
    public class MagicSatDishEventPanel : RustPlugin
    {
        #region Class Fields
        [PluginReference] private readonly Plugin MagicPanel, SatDishEvent;

        private PluginConfig _pluginConfig; //Plugin Config
        private int _activeSatDishEvent;
        ...
        private PluginConfig AdditionalConfig(PluginConfig config)
        {
            config.Panel = new Panel
            {
                Image = new PanelImage
                {
                    ...
                    Url = config.Panel?.Image?.Url ?? "https://www.dropbox.com/s/md6grqalil26sbk/SatelliteDishEvent.png?dl=1",
        ...
        private void MagicPanelRegisterPanels()
        {
            ...
            if (SatDishEvent == null)
            {
                PrintError("Missing plugin dependency SatDishEvent");
                return;
            }
        ...
        private void OnSatDishEventStart()
        {
            _activeSatDishEvent++;
            if (_activeSatDishEvent == 1)
            {
        ...
        {
            _activeSatDishEvent = Mathf.Min(0, _activeSatDishEvent - 1);
            if (_activeSatDishEvent == 0)
            {
                MagicPanel?.Call("UpdatePanel", Name, (int)UpdateEnum.Image);
            }
        }
        ...
        private Hash<string, object> GetPanel()
        {
            ...
            if (image != null)
            {
                image.Color = _activeSatDishEvent != 0 ? _pluginConfig.ActiveColor : _pluginConfig.InactiveColor;
            }
