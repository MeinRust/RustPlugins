...
{
    [Info("Magic Convoy Event Panel", "MJSU edit by MeinRust", "1.0.0")]
    [Description("Displays if the Convoy event is active and changes the color of the Convoy icon when a Convoy is active.")]
    public class Magic Convoy Event Panel : RustPlugin
    {
        #region Class Fields
        [PluginReference] private readonly Plugin MagicPanel, Convoy;

        private PluginConfig _pluginConfig; //Plugin Config
        private int _activeConvoy;
        ...
        private PluginConfig AdditionalConfig(PluginConfig config)
        {
            config.Panel = new Panel
            {
                Image = new PanelImage
                {
                    ...
                    Url = config.Panel?.Image?.Url ?? "https://www.dropbox.com/s/16qx5cj1ql7kyka/convoynew.png?dl=1",
        ...
        private void MagicPanelRegisterPanels()
        {
            ...
            if (Convoy == null)
            {
                PrintError("Missing plugin dependency Convoy");
                return;
            }
        ...
        {
            _activeConvoy = Mathf.Min(0, _activeConvoy - 1);
            if (_activeConvoy == 0)
            {
                MagicPanel?.Call("UpdatePanel", Name, (int)UpdateEnum.Image);
            }
        ...
        private Hash<string, object> GetPanel()
        {
            ...
            if (image != null)
            {
                image.Color = _activeConvoy != 0 ? _pluginConfig.ActiveColor : _pluginConfig.InactiveColor;
            }
