...
{
    [Info("Magic Harbor Event Panel", "MJSU edit MeinRust", "1.0.0")]
    [Description("Displays if the Harbor event is active and changes the color of the Harbor icon when a Harbor is active.")]
    public class MagicHarborEventPanel : RustPlugin
    {
        #region Class Fields
        [PluginReference] private readonly Plugin MagicPanel, HarborEvent;

        private PluginConfig _pluginConfig; //Plugin Config
        private int _activeHarborEvent;
        ...
        private PluginConfig AdditionalConfig(PluginConfig config)
        {
            config.Panel = new Panel
            {
                Image = new PanelImage
                {
                    ...
                    Url = config.Panel?.Image?.Url ?? "https://www.dropbox.com/s/n9rnkz8cghzt9n4/HarborEvent.png?dl=1",
        ...
        private void MagicPanelRegisterPanels()
        {
            ...
            if (HarborEvent == null)
            {
                PrintError("Missing plugin dependency HarborEvent");
                return;
            }
        ...
        private void OnHarborEventStart()
        {
            _activeHarborEvent++;
            if (_activeHarborEvent == 1)
            {
        ...
        {
            _activeHarborEvent = Mathf.Min(0, _activeHarborEvent - 1);
            if (_activeHarborEvent == 0)
            {
                MagicPanel?.Call("UpdatePanel", Name, (int)UpdateEnum.Image);
            }
        }
        ...
        private Hash<string, object> GetPanel()
        {
            ...
            if (image != null)
            {
                image.Color = _activeHarborEvent != 0 ? _pluginConfig.ActiveColor : _pluginConfig.InactiveColor;
            }
