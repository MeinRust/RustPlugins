 ...
 *
 * v1.0.0   :  RaidableBase event support 
 * Icon by Freepik
 *
 **********************************************************************/
#endregion
...
{
    [Info("Magic RaidableBase Panel", "MJSU edit by MeinRust", "1.0.0")]
    [Description("Displays if the RaidableBase event is active and changes the color of the RaidableBase icon when a RaidableBase is active.")]
    public class MagicRaidableBasePanel : RustPlugin
    {
        #region Class Fields
        [PluginReference] private readonly Plugin MagicPanel, RaidableBases;

        private PluginConfig _pluginConfig; //Plugin Config
        private int _activeRaidableBases;
        ...
        private PluginConfig AdditionalConfig(PluginConfig config)
        {
            config.Panel = new Panel
            {
                Image = new PanelImage
                {
                    ...
                    Url = config.Panel?.Image?.Url ?? "https://www.dropbox.com/s/b074ymvcxypljm7/RaidBase.png?dl=1",
        ...
        private void MagicPanelRegisterPanels()
        {
            ...
            {
                PrintError("Missing plugin dependency RaidableBases");
                return;
            }
        ...
        private void OnRaidableBaseStarted()
        {
            _activeRaidableBases++;
            if (_activeRaidableBases == 1)
            {
        ...
        {
            _activeRaidableBases = Mathf.Min(0, _activeRaidableBases - 1);
            if (_activeRaidableBases == 0)
            {
                MagicPanel?.Call("UpdatePanel", Name, (int)UpdateEnum.Image);
            }
        }
        ...
        private Hash<string, object> GetPanel()
        {
            ...
            if (image != null)
            {
                image.Color = _activeRaidableBases != 0 ? _pluginConfig.ActiveColor : _pluginConfig.InactiveColor;
            }
