...
{
    [Info("Magic Water Event Panel", "MJSU edit by MeinRust", "1.0.0")]
    [Description("Displays if the Water event is active and changes the color of the Water icon when a Water is active.")]
    public class MagicWaterEventPanel : RustPlugin
    {
        #region Class Fields
        [PluginReference] private readonly Plugin MagicPanel, WaterEvent;

        private PluginConfig _pluginConfig; //Plugin Config
        private int _activeWaterEvent;
        ...
        private PluginConfig AdditionalConfig(PluginConfig config)
        {
            config.Panel = new Panel
            {
                Image = new PanelImage
                {
                    ...
                    Url = config.Panel?.Image?.Url ?? "https://www.dropbox.com/s/brcgbymy1ln4fcp/WaterEvent.png?dl=1",
        ...
        private void MagicPanelRegisterPanels()
        {
            ...
            if (WaterEvent == null)
            {
                PrintError("Missing plugin dependency WaterEvent");
                return;
            }
        ...
        private void OnWaterEventStart(HashSet<BaseEntity> entities)
        {
            _activeWaterEvent++;
            if (_activeWaterEvent == 1)
            {
        ...
        {
            _activeWaterEvent = Mathf.Min(0, _activeWaterEvent - 1);
            if (_activeWaterEvent == 0)
            {
                MagicPanel?.Call("UpdatePanel", Name, (int)UpdateEnum.Image);
            }
        }
        ...
        private Hash<string, object> GetPanel()
        {
            ...
            if (image != null)
            {
                image.Color = _activeWaterEvent != 0 ? _pluginConfig.ActiveColor : _pluginConfig.InactiveColor;
            }
